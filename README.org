...
| =password_file=      | a raw text file that contains the account's password for authentication. It must be the sole content of the file.                          | <<orgid-yiitzk-password_file>>      |
| =client_id=          | the app Client ID from "https://www.reddit.com/prefs/apps".                                                                                | <<orgid-yiitzk-client_id>>          |
| =client_secret_file= | a raw text file that contains the the app Client Secret from "https://www.reddit.com/prefs/apps". It must be the sole content of the file. | <<orgid-yiitzk-client_secret_file>> |
